...
import pro.cyrent.anticheat.util.ray.blocks.*;
...
public enum BlockData {
    ...
    _LIQUID(new SimpleCollisionBox(0, 0, 0, 1, 0.9, 1),
            ...
            MathUtil.match("STATIONARY_LAVA"), MathUtil.match("STATIONARY_WATER")),
    ...
    }, MathUtil.match("CAKE"), MathUtil.match("CAKE_BLOCK")),
    ...
    _FENCE(new DynamicFence(), Arrays.stream(XMaterial.values())
            ...
            .map(BlockData::m)
            .toArray(Material[]::new)),
    _PANE(new DynamicPane(), MathUtil.match("THIN_GLASS"), MathUtil.match("STAINED_GLASS_PANE"),
            MathUtil.match("IRON_FENCE")),
    ...
    _ETABLE(new SimpleCollisionBox(0, 0, 0, 1, 1 - 0.25, 1),
            MathUtil.match("ENCHANTMENT_TABLE")),
    _FRAME(new SimpleCollisionBox(0, 0, 0, 1, 1 - (0.0625 * 3), 1),
            MathUtil.match("ENDER_PORTAL_FRAME")),

    _CARPET(new SimpleCollisionBox(0.0F, 0.0F, 0.0F, 1.0F, 0.0625F, 1.0F), MathUtil.match("CARPET")),
    _Daylight(new SimpleCollisionBox(0.0F, 0.0F, 0.0F, 1.0F, 0.375, 1.0F),
            MathUtil.match("DAYLIGHT_DETECTOR"), MathUtil.match("DAYLIGHT_DETECTOR_INVERTED")),
    _LILIPAD((v, b) -> {
        if (v < 19) return new SimpleCollisionBox(0.0f, 0.0F, 0.0f, 1.0f, 0.015625F, 1.0f);
        return new SimpleCollisionBox(0.0625, 0.0F, 0.0625, 0.9375, 0.015625F, 0.9375);
    }, MathUtil.match("WATER_LILY")),
    ...
    _STUPID(new SimpleCollisionBox(0.0F, 0.0F, 0.0F, 1.0F, 0.125F, 1.0F),
            MathUtil.match("DIODE_BLOCK_OFF"), MathUtil.match("DIODE_BLOCK_ON"),
            MathUtil.match("REDSTONE_COMPARATOR_ON"), MathUtil.match("REDSTONE_COMPARATOR_OFF")),
    ...
    public CollisionBox getBox(Block block, int version) {
        if (this.box != null)
    ...
    public static BlockData getData(Material material) {
        Material matched = MathUtil.match(material.toString());
