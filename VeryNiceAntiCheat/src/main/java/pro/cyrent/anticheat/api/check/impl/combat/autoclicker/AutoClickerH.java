    ...
    public void onPacket(PacketEvent event) {
        ...
        if (event.getPacketReceiveEvent().getPacketType() == PacketType.Play.Client.PLAYER_BLOCK_PLACEMENT) {
            ...
            if (this.movements < 20) {
                ...
                if (clickSamples.size() >= 90) {
                    double standardDeviation = StreamUtil.getStandardDeviation(this.clickSamples);


                    if (standardDeviation < .38) {
                        if (++this.threshold > 1.5) {
                            this.fail("std="+standardDeviation);
                        }
                    } else {
